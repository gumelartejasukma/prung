...
function addEvent(req,res){
  ...
  db.collection(EVENTS_COLLECTION).insertOne(event, function(err, doc) {
    if (err) {
      ...
      db.collection(USERS_COLLECTION).findOneAndUpdate({ _id: new ObjectID(body.user_id) },{$push:{events:doc.ops[0]._id.valueOf()}},{safe:true,upsert:true},function(err2, doc2) {
        if (err2) {
